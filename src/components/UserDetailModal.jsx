...
const UserDetailModal = ({ user, isOpen, onClose, onUnlock }) => {
  if (!isOpen || !user) return null;

  return (
    <div className="fixed inset-0 bg-navy-900/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
      <div className="bg-white rounded-[3.5rem] w-full max-w-4xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-500 border border-white/20">
        <div className="bg-slate-50 px-12 py-10 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <div className="w-20 h-20 bg-navy-900 rounded-[2rem] flex items-center justify-center font-black text-electric text-2xl shadow-2xl border-4 border-white">
          ...
          <button onClick={onClose} className="w-12 h-12 flex items-center justify-center rounded-3xl bg-white border border-slate-200 text-slate-400 hover:text-navy-900 transition-colors shadow-sm"><X size={20}/></button>
        </div>
        ...
        <div className="p-12 overflow-y-auto max-h-[70vh]">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <p className="text-sm font-bold font-mono text-slate-600 bg-slate-100 px-3 py-2 rounded-xl border border-slate-200">{user.accountNumber}</p>
                </div>
                <div>
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-6">
                <div className="p-5 bg-navy-900 rounded-3xl border border-navy-800 shadow-xl shadow-navy-900/20">
          ...
          {/* Status Banner */}
          <div className={`mt-12 p-8 rounded-[2.5rem] border flex items-center justify-between ${
            user.isLocked ? 'bg-danger/5 border-danger/10 text-danger' : 'bg-success/5 border-success/10 text-success'
          }`}>
            <div className="flex items-center gap-6">
              <div className={`p-4 rounded-2xl ${user.isLocked ? 'bg-danger text-white shadow-xl shadow-danger/20' : 'bg-success text-white shadow-xl shadow-success/20'}`}>
            ...
            {user.isLocked && onUnlock && (
              <button 
                ...
                className="px-10 py-4 bg-danger text-white rounded-2xl font-black italic text-sm hover:bg-navy-900 hover:text-electric transition-all shadow-xl shadow-danger/20 active:scale-95"
              >
        ...
        <div className="px-12 py-8 bg-slate-50 border-t border-slate-100 flex justify-between items-center">
           ...
           <button onClick={onClose} className="px-12 py-4 bg-navy-900 text-white font-black italic rounded-2xl shadow-xl shadow-navy-900/20 hover:bg-electric transition-all active:scale-95">CLOSE VAULT</button>
        </div>
      </div>
    </div>
  );
